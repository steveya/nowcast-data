...
def main() -> None:
    ...
    # Define backtest period: last 10 business days of 2025
    end_date = date(2025, 12, 31)
    start_date = end_date - timedelta(days=30)

    # Generate vintage dates (approximately business days)
    vintage_dates = pd.bdate_range(start=start_date, end=end_date).date.tolist()

    print("=" * 80)
    print("NOWCASTING MODEL BACKTEST")
    print("=" * 80)
    print(f"\nBacktest period: {start_date} to {end_date}")
    print(f"Number of vintage dates: {len(vintage_dates)}")
    ...
    online_config = BacktestConfig(
        ...
        scoring_label="y_asof_latest",
        model="ridge",
        min_train_quarters=20,
        evaluation_asof_date=None,  # Not needed for online label
    )

    results_online = run_backtest(manager, online_config)

    print(f"\nResults shape: {results_online.shape}")
    print("\nSummary statistics:")
    print(results_online[["y_pred", "y_true_asof", "n_train", "n_features"]].describe())

    print("\nFirst 5 predictions:")
    print(
        results_online[["asof_date", "ref_quarter", "y_pred", "y_true_asof", "label_used"]].head()
    )
    ...
    offline_config = BacktestConfig(
        ...
        agg_spec=agg_spec,
        vintages=vintage_dates,
        scoring_label="y_final",
        model="ridge",
        min_train_quarters=20,
        evaluation_asof_date=evaluation_date,
    )

    results_offline = run_backtest(manager, offline_config)
    ...
    comparison = pd.merge(
        results_online[["asof_date", "ref_quarter", "y_pred", "y_true_asof"]].rename(
            columns={"y_pred": "y_pred_online", "y_true_asof": "y_online"}
        ),
